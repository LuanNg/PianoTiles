...
namespace Mio.TileMaster {
    ...
    public class TileMasterGamePlay : MonoBehaviour {
        ...
        IEnumerator<float> WaitForLoadingAds () {
            ...
            if (!Utils.AdHelper.Instance.IsAdsDisabled() && Application.internetReachability != NetworkReachability.NotReachable) {
                //uncomment these line if you want to wait for longer, since when the ads are loading, the game will have hiccups
                //while (!Utils.AdHelper.Instance.IsFullScreenAdsReady() && elapsedTime <= 3) {
