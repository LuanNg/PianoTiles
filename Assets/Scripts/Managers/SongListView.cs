...
namespace Mio.TileMaster
{
    public class SongListView : MonoBehaviour
    {
        ...
        public NativeAdsItem nativeAdItem;
        ...
        public void LoadNativeAds(string nativeAdsID)
        {
            ...
            {
                nativeAdItem.LoadNativeAds(nativeAdsID);
            }
        }
        ...
        private NativeAdsItem SpwanAds(GameObject gameobject)
        {
            //Debug.Log("swapmn ads");
            Transform AdsTf = nativeAdItem.Spawn().transform;
            AdsTf.SetParent(itemScrollList.transform);
            AdsTf.localPosition = new Vector2(0, gameobject.transform.localPosition.y);
            AdsTf.localScale = Vector3.one;
            AdsTf.gameObject.SetActive(true);
            return AdsTf.GetComponent<NativeAdsItem>();
        }
        ...
        private void OnSongItemNeedInitialized(GameObject gameobject, int wrapIndex, int realIndex)
        {
            ...
            if (index == GameManager.Instance.GameConfigs.indexNativeAds[0] - 1)
            {
				...
				if (!isSpawnAdsFirst) {
					//Debug.Log(index);
					NativeAdsItem ads = SpwanAds (gameobject);
					ads.impressionCamera = cameraForImpesstionNativeAds;
#if UNITY_ANDROID
					if (GameManager.Instance.GameConfigs.nativeAdsSlot1 [0] != null) {
                       
						ads.LoadNativeAds (GameManager.Instance.GameConfigs.nativeAdsSlot1 [0]);
					}
#elif UNITY_IOS
                     if (GameManager.Instance.GameConfigs.nativeAdsSlot1[1] != null) 
                    {
                        ads.LoadNativeAds(GameManager.Instance.GameConfigs.nativeAdsSlot1[1]);
                    }
#endif
                    ads1 = ads;
					isSpawnAdsFirst = true;
				} else {
					ads1.LoadCrossPromotion ();
				}
            }
            else if (index == GameManager.Instance.GameConfigs.indexNativeAds[1])
            {
                ...
                if (!isSpawnAdsLast)
                {
                    ...
                    ads.impressionCamera = cameraForImpesstionNativeAds;
#if UNITY_ANDROID
                    if (GameManager.Instance.GameConfigs.nativeAdsSlot2[0] != null)
                    {
                        
                        ads.LoadNativeAds(GameManager.Instance.GameConfigs.nativeAdsSlot2[0]);
                    }
#elif UNITY_IOS
                     if (GameManager.Instance.GameConfigs.nativeAdsSlot2[1] != null) 
                    {
                        ads.LoadNativeAds(GameManager.Instance.GameConfigs.nativeAdsSlot2[1]);
                    }
#endif
                    ads2 = ads;
                    isSpawnAdsLast = true;
